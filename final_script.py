...
def retry_subprocess(command, enable_proxy=False, retries=3):
...
def migrate_specific_branches(repo_dir, github_repo_url, branches):
    ...
    for branch in branches:
        ...
        retry_subprocess(["git", "fetch", "--tags"])
        retry_subprocess(["git", "push", github_repo_url, f"refs/remotes/origin/{branch}:refs/heads/{branch}"], enable_proxy=True)
        retry_subprocess(["git", "push", "--tags", github_repo_url], enable_proxy=True)
        ...
        try:
            ...
            if not only_clone_repos:
                ...
                if specific_branches == "all":
                    ...
                    retry_subprocess(["git", "push", "--mirror", github_repo_url], enable_proxy=True)
                else:
